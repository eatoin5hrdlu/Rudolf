...
unsigned long occ;
volatile unsigned long int lockout; // Don't re-trigger for a while
...
int t,h,f;
	...
	{
		t = digitalRead(TRAIN);
		h = digitalRead(HALFWAY);
		f = digitalRead(FINISHED);
...
void loop()
{
	...
	attachInterrupt(digitalPinToInterrupt(FINISHED),finished,FALLING);
		...
		switch(state) {
			case INBOUND:
				detachInterrupt(digitalPinToInterrupt(HALFWAY));
				set_direction(true);
				motor(phase());
				if (count++ > TIMEOUT) {
					set_direction(false);
			...
			case IDLE:
				...
				if (train) {
					detachInterrupt(digitalPinToInterrupt(TRAIN));
					set_direction(false);
					last_timestamp = millis();
					setstate(OUTBOUND);
